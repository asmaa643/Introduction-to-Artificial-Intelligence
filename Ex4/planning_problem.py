    ...
    def get_successors(self, state):
        ...
        for action in self.actions:
            if action.all_preconds_in_list(state) and not action.is_noop():
                new_state = frozenset(state.union(set(action.add)) - set(action.delete))
...
def max_level(state, planning_problem):
    ...
    goal_levels = {goal: -1 for goal in planning_problem.goal}
    ...
    while True:
        ...
        if is_fixed(graph, level):
            return float('inf')
        remaining_goals = update_next(goal_levels, graph, level, remaining_goals)
        level += 1
...
def update_next(goal_levels, graph, level, remaining_goals):
    current_props_set = set(graph[level].get_proposition_layer().get_propositions())
    found_goals = remaining_goals & current_props_set
    for goal in found_goals:
        if goal_levels[goal] == -1:
            goal_levels[goal] = level
    remaining_goals -= found_goals
