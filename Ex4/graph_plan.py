...
def independent_pair(a1, a2):
    ...
    """
    "*** YOUR CODE HERE ***"
    a1_pre = a1.get_pre() # returns the pre list of a1
    a1_add = a1.get_add() #returns the add list of a1
    a1_del = a1.get_delete() #return the del list of a1
    a2_pre = a2.get_pre()  # returns the pre list of a2
    a2_add = a2.get_add()  # returns the add list of a2
    a2_del = a2.get_delete()  # return the del list of a2

    return (a1 == a2) or \
        (not np.any(np.isin(a1_add, a2_del)) and
        not np.any(np.isin(a2_add, a1_del)) and
        not np.any(np.isin(a1_pre, a2_del)) and
        not np.any(np.isin(a2_pre, a1_del)))
